...
/* virtual */ void FsLazyWindowApplication::Interval(void)
{
	...
	if(evt==FSMOUSEEVENT_LBUTTONDOWN)
	{
		...
		if (state=='p')
		{
			Paint(mesh, plHd, 20.0);
		}
		RemakeVertexArray();
	}
	if(key==FSKEY_S)
	{
		vtxArray =  MakeSliceVertexArray(mesh);
		state = 's';
		printf("vector size: %d\n", vtxArray.size());
	}
	if(key==FSKEY_L)
	{
		vtxArrayR = MakeRayIntersectionVertexArray(mesh);
		state = 'l';
		printf("vector size: %d\n", vtxArrayR.size());
	}
...
/* virtual */ void FsLazyWindowApplication::Draw(void)
{
	...
	{
		glBegin(GL_LINES);
		for (int i=0; i<vtxArray.size()-6; i+=6)
		{
			glVertex3f(vtxArray[i+0], vtxArray[i+1],vtxArray[i+2]);
			glVertex3f(vtxArray[i+3], vtxArray[i+4],vtxArray[i+5]);
		}
		glEnd();
	}
	...
	if (state=='l')
	{
		glBegin(GL_POINTS);
		glPointSize(4);
		for (int i=0; i<vtxArrayR.size()-3; i+=3)
		{
			glVertex3f(vtxArrayR[i+0], vtxArrayR[i+1], vtxArrayR[i+2]);
		}
		glEnd();
	}
