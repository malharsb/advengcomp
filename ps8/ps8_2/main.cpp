...
/* virtual */ void FsLazyWindowApplication::Initialize(int argc,char *argv[])
{
	if(2<=argc && true==mesh.LoadStl(argv[1]))
	{
		...
		YsShellExt::VertexHandle xmin=nullptr,xmax=nullptr,ymin=nullptr,ymax=nullptr,zmin=nullptr,zmax=nullptr;
		for(auto vtHd : mesh.AllVertex())
		{
			auto pos=mesh.GetVertexPosition(vtHd);
			if(nullptr==xmin || pos.x()<mesh.GetVertexPosition(xmin).x())
			{
				xmin=vtHd;
			}
			if(nullptr==xmax || pos.x()>mesh.GetVertexPosition(xmax).x())
			{
				xmax=vtHd;
			}
			if(nullptr==ymin || pos.y()<mesh.GetVertexPosition(ymin).y())
			{
				ymin=vtHd;
			}
			if(nullptr==ymax || pos.y()>mesh.GetVertexPosition(ymax).y())
			{
				ymax=vtHd;
			}
			if(nullptr==zmin || pos.z()<mesh.GetVertexPosition(zmin).z())
			{
				zmin=vtHd;
			}
			if(nullptr==zmax || pos.z()>mesh.GetVertexPosition(zmax).z())
			{
				zmax=vtHd;
			}
		}

		std::vector <YsShellExt::VertexHandle> astarPath[3];
		if(nullptr!=xmin)
		{
			astarPath[0]=A_Star(mesh,xmin,xmax);
			astarPath[1]=A_Star(mesh,ymin,ymax);
			astarPath[2]=A_Star(mesh,zmin,zmax);
			for(int i=0; i<3; ++i)
			{
				for(auto vtHd : astarPath[i])
				{
					pathVtx[i].push_back(mesh.GetVertexPosition(vtHd).x());
					pathVtx[i].push_back(mesh.GetVertexPosition(vtHd).y());
					pathVtx[i].push_back(mesh.GetVertexPosition(vtHd).z());
				}
			}
		}
	}
...
/* virtual */ void FsLazyWindowApplication::Draw(void)
{
	...
	glColor3ub(0,0,0);
	for(auto &vtx : pathVtx)
	{
		glEnableClientState(GL_VERTEX_ARRAY);
		glVertexPointer(3,GL_FLOAT,0,vtx.data());
		glDrawArrays(GL_LINE_STRIP,0,vtx.size()/3);
		glDisableClientState(GL_VERTEX_ARRAY);
	}
