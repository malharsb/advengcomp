...
{
	if(2<=argc) // DO WE INCLUDE THE BINARY STATEMENT???
	{
		// Read input arguments and get profile from Makeairfoil
		naca = atoi(argv[1]);
		s = atof(argv[2]);
		b = atof(argv[3]);
		di = atof(argv[4]);
		p = atof(argv[5]);
		dz = atof(argv[6]);
		printf("naca: %d s: %f b: %f di: %f p: %f dz: %f\n",naca,s,b,di,p,dz);

		// Create the mesh
		MakeNACA4DigitAirfoilWing(mesh, naca, s, b, di, p, dz);

		// Save stl
		char fn[] = "wing.stl";
		mesh.SaveBinStl(fn); //HOW ON EARTH DO YOU WRITE THIS????
