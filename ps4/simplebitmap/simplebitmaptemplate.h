...
public:
	...
	*/
	void CutOut(SimpleBitmapTemplate <ComponentType,NumComponentPerPixel> &destination,int thisX0,int thisY0,int wid,int hei,ComponentType clearColor) const
	{
		...
		// Follow syntaxes from Create because destination already exists
		destination.CleanUp(); //???
		destination.nx=wid;
		destination.ny=hei;
		...
		for (int r=thisX0, r_dst=0; r<thisX0+wid; r++, r_dst++)
		{
			for (int c=thisY0, c_dst=0; c<thisY0+hei; c++, c_dst++)
			{

				if (IsInRange(r,c))
				{
					const ComponentType *srcptr = GetPixelPointer(r, c);
					ComponentType *dstptr = destination.GetEditablePixelPointer(r_dst, c_dst);

					for (int i=0; i<NumComponentPerPixel; i++)
						dstptr[i] = srcptr[i];
				}
				else
				{
