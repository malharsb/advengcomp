...
bool SimpleBitmap::operator==(const SimpleBitmap &bitmapB) const
{
	...
	if (this->GetHeight() != bitmapB.GetHeight()) return false;
	if (this->GetWidth() != bitmapB.GetWidth()) return false;
	...
	for (int i=0; i<GetTotalNumComponent(wid, hei); i++)
